...
def trouver_proches():
    ...
    # 1. On récupère d'abord la position du Marais dans 'delivery_points' [cite: 205]
    ...
    # 2. On cherche les livreurs dans 'drivers_locations' autour de ces coordonnées [cite: 211-212]
...
def affectation_optimale(lieu="Marais"):
    ...
    # 1. Récupérer la position du lieu [cite: 205]
    pos = r.geopos("delivery_points", lieu)
    if not pos or not pos[0]: return
    lon, lat = pos[0]

    # 2. Trouver les livreurs dans un rayon de 3 km [cite: 216]
    ...
    for d_id, dist in candidats:
        # 3. Récupérer le rating depuis le Hash du livreur [cite: 70, 217]
